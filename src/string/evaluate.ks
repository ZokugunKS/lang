		...
		if value.startsWith('function') || value.startsWith('{') || context? {
			if context is Object {
				if value.startsWith('return') || hasReturn {
					return eval('(function(' + Object.keys(context).join(',') + '){' + value + '})').apply(null, Object.values(context))
				}
				else {
					return eval('(function(' + Object.keys(context).join(',') + '){return ' + value + ';})').apply(null, Object.values(context))
				}
			}
			else {
